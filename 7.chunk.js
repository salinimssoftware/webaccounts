...
DashboardModule = __decorate([
    __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_0__angular_core__["NgModule"])({
        ...
        declarations: [
            ...
            __WEBPACK_IMPORTED_MODULE_6__fiscalyear_fiscalyear_component__["a" /* fiscalyearComponent */]
        ],
        providers: []
    })
], DashboardModule);
...
var Dashboard = (function () {
    function Dashboard(state, router, masterService, alertService, authService) {
        this.state = state;
        this.router = router;
        this.masterService = masterService;
        this.alertService = alertService;
        this.authService = authService;
        this.subsriptionArray = [];
        this.totalIncome = 0;
        this.totalexpenses = 0;
        this.netsalesamount = 0;
        this.salesamount = 0;
        this.salesreturnamount = 0;
        ...
        this.netvat = 0;
        this.vatpayable = 0;
        this.vatreceivable = 0;
        this.userProfile = {};
        this.userSetting = {};
        this.incomeexpensereportmode = 0;
        this.receivepayementreportmode = 0;
        this.expensesreportmode = 0;
        this.reportmode = 0;
        this.fiscalForm = new __WEBPACK_IMPORTED_MODULE_8__angular_forms__["FormGroup"]({
            StartDate_AD: new __WEBPACK_IMPORTED_MODULE_8__angular_forms__["FormControl"]("", [__WEBPACK_IMPORTED_MODULE_8__angular_forms__["Validators"].required]),
            EndDate_AD: new __WEBPACK_IMPORTED_MODULE_8__angular_forms__["FormControl"]("", [__WEBPACK_IMPORTED_MODULE_8__angular_forms__["Validators"].required]),
            FiscalID: new __WEBPACK_IMPORTED_MODULE_8__angular_forms__["FormControl"]("", [__WEBPACK_IMPORTED_MODULE_8__angular_forms__["Validators"].required])
        });
        this.subcriptions = [];
        var today = new Date();
        this.Currdate = __WEBPACK_IMPORTED_MODULE_7_moment__(today).format('YYYY-MM-DD');
        this.userProfile = authService.getUserProfile();
        this.userSetting = authService.getSetting();
        this.userProfile = this.authService.getUserProfile();
        var dashboardright = this.userProfile.userRights.find(function (x) { return x.right == "ShowDashboard"; });
    ...
    Dashboard.prototype.IncomeExpensesAccounting = function () {
        ...
        this.masterService.getIncomeExpensesAccounting(this.Currdate, this.userProfile.userDivision, this.userProfile.PhiscalYearInfo.PhiscalID, this.incomeexpensereportmode).subscribe(function (data) {
            // console.log('incomeexpenses', data);
            var fordatelabels = data[0];
            //  this.receivepayment= data[0].2022-06-18;
            // console.log('purchasedatevalue', data);
            _this.salespurchaselabels = [];
            for (var attribute in fordatelabels) {
                if (['A'].indexOf(attribute) == -1) {
                    _this.salespurchaselabels.push(attribute);
                }
            }
            var dataset = [];
            for (var _i = 0, data_1 = data; _i < data_1.length; _i++) {
                var datavalue = data_1[_i];
                var tempsalpurData = {};
                tempsalpurData.data = [];
                ...
                for (var attri in datavalue) {
                    if (['A'].indexOf(attri) == -1) {
                        ...
                        var num = parseFloat(datavalue[attri]);
                        // console.log("num", num)
            ...
            window.myCharts3 = new __WEBPACK_IMPORTED_MODULE_5_chart_js__(_this.ctx, {
                type: 'line',
                data: {
                    datasets: dataset,
                    labels: _this.salespurchaselabels
                },
            });
        });
    };
    ...
    //   });
    // }
    Dashboard.prototype.getReceivePaymentforchartata = function () {
        var _this = this;
        ...
        this.masterService.getReceivePaymentforchart(this.Currdate, this.userProfile.userDivision, this.userProfile.PhiscalYearInfo.PhiscalID, this.receivepayementreportmode)
            ...
            var fordatelabels = data[0];
            //  this.receivepayment= data[0].2022-06-18;
            _this.receivepaymentLabels = [];
            for (var attribute in fordatelabels) {
                if (['a'].indexOf(attribute) == -1) {
                    _this.receivepaymentLabels.push(attribute);
                }
            }
            // console.log("datelabelsss", this.receivepaymentLabels);
            var dataset = [];
            for (var _i = 0, data_2 = data; _i < data_2.length; _i++) {
                var datavalue = data_2[_i];
                var tempData = {};
                tempData.data = [];
                tempData.backgroundColor = [];
                tempData.label = datavalue.a ? datavalue.a : datavalue.A;
                for (var attri in datavalue) {
                    if (['a'].indexOf(attri) == -1 || ['A'].indexOf(attri) == -1) {
                        // console.log("attri", attri, parseFloat(datavalue[attri]));
                        var num = parseFloat(datavalue[attri]);
                        // console.log("num",num)
                        tempData.data.push(num);
                        var bgcolor = '#00ACED';
                        if (datavalue.a == 'Payment Value' || datavalue.A == 'PAYMENT') {
                            bgcolor = '#E06DDE';
                        }
            ...
            window.myCharts2 = new __WEBPACK_IMPORTED_MODULE_5_chart_js__(_this.ctx3, {
                type: 'bar',
                data: {
                    labels: _this.receivepaymentLabels,
                    datasets: dataset,
                },
            });
    ...
    Dashboard.prototype.dashboardmaindata = function () {
        var _this = this;
        this.masterService.getdashboardmaindata(this.Currdate, this.userProfile.userDivision, this.userProfile.PhiscalYearInfo.PhiscalID)
            .subscribe(function (mainData) {
            // console.log('chartdata', mainData);
            // if (mainData && mainData?.length > 0) {
            _this.totalIncome = _this.masterService.nullToZeroConverter(mainData[0].TOTALINCOME);
            _this.totalexpenses = _this.masterService.nullToZeroConverter(mainData[0].TOTALEXPENSES);
            _this.netsalesamount = _this.masterService.nullToZeroConverter(mainData[0].NETSALESAMOUNT);
            _this.salesamount = _this.masterService.nullToZeroConverter(mainData[0].SALESAMOUNT);
            _this.salesreturnamount = _this.masterService.nullToZeroConverter(mainData[0].SALESRETURNAMOUNT);
            _this.cashinhand = _this.masterService.nullToZeroConverter(mainData[0].CASH);
            _this.cashatbank = _this.masterService.nullToZeroConverter(mainData[0].BANK);
            _this.receipt = _this.masterService.nullToZeroConverter(mainData[0].RECEIPT);
            _this.payment = _this.masterService.nullToZeroConverter(mainData[0].PAYMENT);
            _this.receivable = _this.masterService.nullToZeroConverter(mainData[0].RECEIVABLE);
            _this.payable = _this.masterService.nullToZeroConverter(mainData[0].PAYABLE);
            _this.netvat = _this.masterService.nullToZeroConverter(mainData[0].NETVAT);
            _this.vatpayable = _this.masterService.nullToZeroConverter(mainData[0].VATPAYABLE);
            _this.vatreceivable = _this.masterService.nullToZeroConverter(mainData[0].VATRECEIVABLE);
            // }
        });
    };
    Dashboard.prototype.getdata = function () {
        ...
        this.masterService.getDetailsforchartjs(this.Currdate, this.userProfile.userDivision, this.userProfile.PhiscalYearInfo.PhiscalID)
            ...
            if (data[0].DEBTORS == null) {
                var notnulldebtors = data[0].DEBTORS;
                var nulltozerodebtors = (notnulldebtors);
                _this.debtorsValue = nulltozerodebtors;
            }
            else {
                var myNum1 = +(data[0].DEBTORS.replace(/,/g, ""));
                _this.debtorsValue = myNum1;
            }
            if (data[0].CREDITORS == null) {
                var notnullcreditors = data[0].CREDITORS;
                var nulltozerocreditors = (notnullcreditors);
                _this.creditorsValue = nulltozerocreditors;
            }
            else {
                var myNum1 = +(data[0].CREDITORS.replace(/,/g, ""));
                _this.creditorsValue = myNum1;
            }
            if (window.myCharts != undefined)
                window.myCharts.destroy();
            window.myCharts = new __WEBPACK_IMPORTED_MODULE_5_chart_js__(_this.ctx4, {
                // new chartJs(this.ctx4, {
                type: 'pie',
                data: {
                    datasets: [{
                            label: 'My First Dataset',
                            data: [_this.debtorsValue, _this.creditorsValue],
                            // data: [100, 200],
                            backgroundColor: [
                                'rgb(255, 99, 132)',
                                'rgb(54, 162, 235)'
            ...
            if ((_this.creditorsValue == 0 || null) && (_this.debtorsValue == 0 || null)) {
                _this.stockGraph = true;
            }
    ...
    Dashboard.prototype.getoverallchartapidata = function () {
        this.getdata();
    ...
    Dashboard.prototype.ngOnDestroy = function () {
        this.subsriptionArray.forEach(function (sub) {
            sub.unsubscribe();
        });
    };
    ...
    Dashboard.prototype.cancelFiscalForm = function () {
        this.authService.showfiscalPopup = false;
    };
    Dashboard.prototype.setDefault = function () {
        ...
        var defaultDate = {
            StartDate_AD: __WEBPACK_IMPORTED_MODULE_7_moment__(this.userProfile.CompanyInfo.FEDATE).add(1, 'days').format('YYYY-MM-DD'),
            EndDate_AD: __WEBPACK_IMPORTED_MODULE_7_moment__(this.userProfile.CompanyInfo.FEDATE).add(1, 'year').format('YYYY-MM-DD'),
            FiscalID: phiscalid
        };
        var fiscalYearEndDate = __WEBPACK_IMPORTED_MODULE_7_moment__(defaultDate.EndDate_AD).format('YYYY');
        var checkLeapYears = this.leapYear(fiscalYearEndDate);
        var newEndDate;
        if (checkLeapYears) {
            newEndDate = __WEBPACK_IMPORTED_MODULE_7_moment__(defaultDate.EndDate_AD).subtract(1, "days").format('YYYY-MM-DD');
    ...
    Dashboard.prototype.leapYear = function (year) {
    ...
    Dashboard.prototype.onSubmit = function () {
        var _this = this;
        ...
        try {
            var saveModel = Object.assign({}, this.fiscalForm.value);
            var sub = this.masterService.saveFiscalYear(saveModel, this.StartDate_BS, this.EndDate_BS).subscribe(function (data) {
                if (data.status == "ok") {
                    _this.alertService.success("Fiscal Year Data Saved Successfully");
                    _this.authService.showfiscalPopup = false;
                }
                else {
    ...
    Dashboard.prototype.changeEntryDate = function (value, format) {
        var adbs = __webpack_require__(19);
        if (format == "AD") {
            var adDate = (value.replace("-", "/")).replace("-", "/");
            var bsDate = adbs.ad2bs(adDate);
            this.StartDate_BS = this.masterService.toADDate(value);
        }
    };
    Dashboard.prototype.changeEndDate = function (value, format) {
        var adbs = __webpack_require__(19);
        if (format == "AD") {
            var adDate = (value.replace("-", "/")).replace("-", "/");
            var bsDate = adbs.ad2bs(adDate);
            this.EndDate_BS = this.masterService.toADDate(value);
        }
    };
    return Dashboard;
}());
...
], Dashboard.prototype, "line", void 0);
__decorate([
    __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_0__angular_core__["ViewChild"])('bar'),
    __metadata("design:type", Object)
], Dashboard.prototype, "bar", void 0);
__decorate([
    __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_0__angular_core__["ViewChild"])('chart4'),
    __metadata("design:type", Object)
], Dashboard.prototype, "chart4", void 0);
...
var routes = [
    {
        ...
        children: [
            { path: 'dashboard', component: __WEBPACK_IMPORTED_MODULE_1__dashboard_component__["a" /* Dashboard */] },
        ]
    },
];
...
var fiscalyearComponent = (function () {
    function fiscalyearComponent(authservice) {
        this.userProfile = {};
    ...
    fiscalyearComponent.prototype.ChangeFY = function (value) {
        // if(value=='changeFY')
        // {
    ...
    fiscalyearComponent.prototype.checkBookClose = function (value) {
        this.FYClose = value == 0 ? "No" : "Yes";
    };
    fiscalyearComponent.prototype.onClickChangeFY = function () {
    ...
    fiscalyearComponent.prototype.onClickCancelFY = function () {
        this.showFiscalYearPopUp = false;
        this.FY = this.FYLIST[0].PhiscalID;
    };
    fiscalyearComponent.prototype.changeFY = function (value) {
        for (var _i = 0, _a = this.PhiscalYearList; _i < _a.length; _i++) {
            var i = _a[_i];
